...
# active. A venv is just a folder containing its own Python + packages; running
# plain `python3` uses a DIFFERENT Python that won't have them installed.
_venv_reminder() {
  # Already inside an active venv? Nothing to say.
  [[ -n $VIRTUAL_ENV ]] && { _venv_warned=; return }

  local d
  for d in .venv venv env; do
    if [[ -x $PWD/$d/bin/python ]]; then
      # Don't repeat the warning for a directory we just warned about.
      [[ $_venv_warned == $PWD ]] && return
      _venv_warned=$PWD
      print -P "%F{yellow}⚠  $d present but not active%f"
      print -P "%F{244}   run: %f%F{cyan}source $d/bin/activate%f%F{244}  (or: $d/bin/python script.py)%f"
      return
    fi
  done
}
autoload -Uz add-zsh-hook
add-zsh-hook chpwd _venv_reminder
# Also check the directory the shell starts in.
_venv_reminder
# -----------------------------------------------------------------------------
...
# work-vs-personal settings) goes in ~/.zshrc.local, which is never committed.
